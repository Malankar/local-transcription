  ...
  test('start/stop, timer, completion card, open in library', async () => {
    ...
    try {
      ...
      try {
        ...
        test.skip(
          true,
          'No downloaded transcription model and/or microphone source (Start Recording stays disabled).',
        )
      }
      ...
      if (await emptyLibrary.isVisible().catch(() => false)) {
        test.skip(true, 'No session saved after stop; capture may need audible speech or a working mic.')
      }
