 ...
 * Summary + transcript Q&A + Thinking mode: Qwen 3 8B — hybrid thinking/non-thinking model;
 * supports Ollama `think` param, so it handles both standard and thinking-mode requests.
 *
...
/**
 * Summary, transcript Q&A, and thinking-mode Q&A. Qwen 3 8B is a hybrid model that supports
 * Ollama's `think` parameter (thinking can be toggled on/off via `think: true/false`).
 *
 * @see https://ollama.com/library/qwen3
 * @see https://docs.ollama.com/capabilities/thinking
 */
export const ASSISTANT_OLLAMA_MODEL_CHAT = 'qwen3:8b'

/** Alias kept for backwards-compat; both modes use the same qwen3:8b model. */
export const ASSISTANT_OLLAMA_MODEL_CHAT_THINKING = ASSISTANT_OLLAMA_MODEL_CHAT
...
export const ASSISTANT_OLLAMA_MODELS_TO_PULL = [
  ...
  { id: ASSISTANT_OLLAMA_MODEL_CHAT, role: 'Summary, chat & thinking mode' },
] as const
