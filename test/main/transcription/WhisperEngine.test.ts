...
describe('WhisperEngine', () => {
  ...
  describe('transcription', () => {
    ...
    it('sends transcribe request and returns results', async () => {
      const chunk: AudioChunk = { audio: new Float32Array(10), startMs: 0, endMs: 100 }
      ...
      const requestId = mockChild.send.mock.calls[0][0].requestId
      const mockSegments = [{ id: '1', text: 'Hello', startMs: 0, endMs: 100, timestamp: 'T1' }]
      mockChild.emit('message', { type: 'result', requestId, segments: mockSegments })
    ...
    it('rejects if worker reports an error', async () => {
      ...
      const requestId = mockChild.send.mock.calls[0][0].requestId
      mockChild.emit('message', { type: 'error', requestId, message: 'Transcription failed' })
    ...
    it('rejects if worker exits unexpectedly', async () => {
      ...
      await expect(transcribePromise).rejects.toThrow('Whisper worker exited unexpectedly')
    })
  })
  ...
  describe('dispose', () => {
    ...
    it('rejects all pending requests when disposed', async () => {
      ...
      await expect(transcribePromise).rejects.toThrow('Whisper worker disposed')
    })
  })
  ...
  describe('setModel', () => {
    ...
    it('disposes the current worker when model changes', () => {
      ...
      const differentModel = { ...mockModel, id: 'large', runtimeModelName: 'large' }
